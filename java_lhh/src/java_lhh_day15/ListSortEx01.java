...
public class ListSortEx01 {
	...
	public static void main(String[] args) {
		 ...
		 * - 아래 코드에서 String 클래스는 라이브러리에서 제공하는 클래스이기 때문에 우리가 수정 불가,
		 * 그래서 익명 클래스(이름이 없는 1회용 클래스)를 만들어서 해당 클래스의 객체를 이용.
		 * - Comparator 인터페이스의 compare 추상 메소드를 오버라이딩 해야하고
		 * 오버라이딩한 코드에 따라 정렬 방식이 바뀜.
		 */
		Collections.sort(list, new Comparator<String>() {
			@Override
			public int compare(String o1, String o2) {
				return -o1.compareTo(o2); // 역순으로 할거면 앞에 -를 붙여준다
			}
		});
		System.out.println(list);
		...
		stds.add(new Student("2024160003", "둘리"));
