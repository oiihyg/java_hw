	...
	public static void main(String[] args) {
		...
		for(Shape tmp : shapes) {
			...
			if(tmp instanceof Circle) {
				Circle circle = (Circle)tmp; // 다운 캐스팅 : 업 캐스팅 된 객체를 다운 캐스팅 할때만 가능
				circle.drawCircle();
				// 두줄 한번에 쓰기
...
class Rect extends Shape{
	...
	int x, y;
	int width, height;
	public Rect(int x, int y, int width, int geight) {
		super("사각형");
		this.x = x;
		this.y = y;
		this.height = height;
	...
	public void print() {
		super.print();
		System.out.println("왼쪽 위에 점 : "+x+","+y);
