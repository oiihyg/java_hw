...
public class StudentManager implements Program{
	...
	private void studentInfoUpdate() {
		...
		if(list.contains(newStudent)) {
			System.out.println("이미 등록된 학생 정보 입니다. 수정 불가능");
			// 삭제했던 학생 정보 추가
			list.add(std);
			return;
		}		
	...
	private void insertSubjectScore() {
		...
		if(subjectList.size()==0) {
			System.out.println("등록된 과목이 없어서 추가할 수 없습니다. 과목을 추가하세요");
			return;
		}
		//학생 정보 입력 후 학생 객체 생성
		//리스트에 입력한 학생 객체가 몇번지에 있는지 번지 가져옴
		Student std = inputStudent();
		int index = list.indexOf(std);
		...
		// 등록된 과목 리스트를 출력
		searchSubject();
		// 학년, 학기, 과목명, 중간, 기발, 수행평가를 입력한 후 과목 객체를 생성
	...
	private void indertSubject() {
		...
		if(subjectList.contains(subject)) {
			System.out.println("d이미 등록된 과목입니다.");
			return;
		}
		// 과목 리스트에 과목을 추가
		subjectList.add(subject);
		System.out.println("과목을 추가햇습니다.");
	}
