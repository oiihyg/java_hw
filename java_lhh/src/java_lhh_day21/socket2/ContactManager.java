...
public class ContactManager implements Program{
	...
	private String ip = "192.168.30.191";
	...
	private void insert() {
		...
		// 중복된 번호인지 확인해서 중복되면 안내문구 출력 후 종료
	...
	private void update() {
		...
		// 수정
		// 수정 : 새로운 연락처가 리스트에 있으면 안내문구 출력 후 종료
		// 단, 현재 선택한 연락처가 아닌 연락처 중에서
		// 전체 탐색
	...
	public void save() {
		...
		// 1. 소켓 생성
		Socket socket = new Socket(ip, port);
		// 2. 소켓을 이용하여 objectoutputstream객체 생성
		ObjectOutputStream oos = new ObjectOutputStream(socket.getOutputStream());
		// 3. save 문자열 전송
		oos.writeUTF("save");
		// 4. 연락처 리시트를 전송
